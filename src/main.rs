...
//Model: USER Sruct with id, name, email
...
fn main(){
    ...
    let listener = TcpListener::bind(format!("0.0.0.0:8880")).unwrap();
    println!("Server started at port 8880");
...
fn handle_delete_request(request: &str) -> (String,String){
    match (get_id(&request).parse::<i32>(), Client::connect(DB_URL, NoTls)) {
        ...
        let _row_affected= client.execute("DELETE FROM users WHERE id = $1", &[&id]).unwrap();
...
//deserialize user from request body with serde
